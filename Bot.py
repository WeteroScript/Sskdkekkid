...
async def work_miner(callback: types.CallbackQuery):
    ...
    if random.random() < 0.3:  # 30% шанс найти ресурс
        ...
        # Сохраняем ресурс в инвентарь пользователя
        ...
        resource_text = f"\n💎 Найден: {selected_resource['name']} (в инвентаре!)"
    
    br_income = add_coinrun_income(user)
    br_text = f"\n🪙 +{br_income} BRcoins (CoinRun)" if br_income > 0 else ""
    
    save_users(users)
    await callback.message.edit_text(f"⛏️ +{income:,}₽{resource_text}{br_text}", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 Назад", callback_data="works")]]))
    await callback.answer()
...
async def buyer_menu(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text(
        ...
        "Сюда можно продать ресурсы, добытые в шахте.\n"
        "Цены на ресурсы:\n\n"
        "1. Красный алмаз - 50,000,000,000₽\n"
        "2. Цветной алмаз - 10,000,000,000₽\n"
        "3. Красная шпинель - 5,000,000,000₽\n"
        "4. Александрит - 2,500,000,000₽\n"
        "5. Рубин - 1,500,000,000₽\n"
        "6. Падпараджа - 750,000,000₽\n"
        "7. Демантоид - 150,000,000₽\n"
        "8. Черный опал - 10,000,000₽\n"
        "9. Танзанит - 5,000,000₽\n"
        "10. Шпинель - 1,500,000₽\n\n"
        "💰 Напишите название ресурса, чтобы продать его.",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 Назад", callback_data="back_main")]])
    )
    await callback.answer()

@dp.message(F.text)
async def handle_sell_resource(message: types.Message):
    if not await check_access(message):
        return
    
    user_id = str(message.from_user.id)
    resource_name = message.text.strip()
    
    # Проверяем, есть ли такой ресурс в списке
    resource = None
    ...
    if not resource:
        await message.answer("❌ Такой ресурс не найден! Введите название из списка.")
        return
    
    # Проверяем, есть ли ресурс у пользователя
    inventory = load_inventory()
    if user_id not in inventory or resource_name not in inventory[user_id]:
        await message.answer(f"❌ У вас нет {resource_name} в инвентаре!")
        return
    
    # Продаем ресурс
    users = load_users()
    user = users[user_id]
    user["money"] += resource["price"]
    user["total_earned"] += resource["price"]
    
    inventory[user_id].remove(resource_name)
    save_inventory(inventory)
    save_users(users)
    
    await message.answer(f"💰 Продано {resource_name} за {resource['price']:,.0f}₽!")
...
async def inventory_main_menu(callback: types.CallbackQuery):
    ...
    text += "\n💰 Чтобы продать ресурс, напишите его название в чате."
...
async def buy_business_menu(callback: types.CallbackQuery):
    ...
    for name, data in BUSINESSES.items():
        owner_count = sum(1 for owner in business_owners.values() if owner == name)
        max_owners = data.get("max_owners", 1)
        
        if owner_count >= max_owners:
            keyboard.append([InlineKeyboardButton(text=f"❌ {name} - занят ({owner_count}/{max_owners})", callback_data=f"biz_info_{name}")])
        else:
            keyboard.append([InlineKeyboardButton(text=f"✅ {name} - {data['price']:,.0f}₽ ({owner_count}/{max_owners})", callback_data=f"biz_choose_{name}")])
...
async def biz_buy_menu(callback: types.CallbackQuery):
    ...
    keyboard = [
        [InlineKeyboardButton(text="✅ Купить", callback_data=f"biz_confirm_{business_name}")],
...
async def biz_info(callback: types.CallbackQuery):
    ...
    business_owners = load_business_owners()
    owner_count = sum(1 for owner in business_owners.values() if owner == business_name)
    max_owners = data.get("max_owners", 1)
