...
last_mine_time = {}  # Для отслеживания КД на шахту
...
async def mine_dig(callback: types.CallbackQuery):
    ...
    if user_id in last_mine_time:
        ...
        if time_diff < 3:
            await callback.answer(f"⏳ Подождите {3 - int(time_diff)} секунд!", show_alert=True)
            return
    ...
    if user["mine_attempts"] <= 0:
        await callback.answer("❌ Нет попыток! Подождите восстановления.", show_alert=True)
        return
...
async def buyer_menu(callback: types.CallbackQuery):
    ...
    # Кнопка "Продать все" в главном меню скупщика
    keyboard.append([InlineKeyboardButton(text="💰 Продать все ресурсы", callback_data="sell_all_resources")])
    keyboard.append([InlineKeyboardButton(text="🔙 Назад", callback_data="back_main")])
