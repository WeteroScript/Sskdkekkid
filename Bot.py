...
async def get_db(message: types.Message):
    ...
    try:
        ...
        files_sent = 0
        for file in [USERS_FILE, PROMOCODES_FILE, INVENTORY_FILE, SETTINGS_FILE]:
...
async def inventory_main_menu(callback: types.CallbackQuery):
    ...
    try:
        ...
        text += "\n💰 Ресурсы можно продать через /sell или в магазине"
...
async def stats_menu(callback: types.CallbackQuery):
    ...
    try:
        ...
        # Получаем количество ресурсов в инвентаре
        inventory = await load_inventory()
        user_id = str(callback.from_user.id)
...
async def sell_command(message: types.Message):
    ...
    try:
        ...
        if user_id not in inventory or not inventory[user_id]:
            await message.answer("❌ У вас нет ресурсов для продажи!")
            return
        ...
        logger.error(f"Ошибка в sell_command: {e}")
...
async def sell_resource_menu(callback: types.CallbackQuery):
    ...
    try:
        ...
        keyboard = [
            ...
            [InlineKeyboardButton(text="🔙 Назад", callback_data="inventory_main")]
        ]
...
async def sell_all_resources(callback: types.CallbackQuery):
    ...
    try:
        ...
        if user_id not in inventory or resource_name not in inventory[user_id]:
            await callback.answer("❌ Нет ресурса!", show_alert=True)
            return
        ...
        await callback.message.edit_text(
            ...
            f"💳 +{total:,.0f}₽",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="🔙 В инвентарь", callback_data="inventory_main")]
            ])
        )
    except Exception as e:
