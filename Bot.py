...
mines_games = {}  # Хранит состояние игр в мины
auto_collect_tasks = {}  # Хранит задачи авто-сбора для пользователей
...
def get_default_user():
    return {
        ...
        "casino": {
            "bet": 0,
            "mines_count": 4,  # Изменено с 3 на 4
            "field_size": 5
        }
    }
...
# ========== АВТО-СБОР БИЗНЕСОВ ==========
async def auto_collect_business(user_id, biz_key):
    """Автоматический сбор бизнеса"""
    try:
        users = await load_users()
        user = users.get(str(user_id), get_default_user())
        ...
        if not user.get("business", {}).get(biz_key, {}).get("owned", False):
            return
        
        if not user.get("business", {}).get(biz_key, {}).get("auto_collect", False):
            return
        
        last_collect = user.get("business", {}).get(biz_key, {}).get("last_collect")
        if not last_collect:
            return
        ...
        if elapsed >= config["cooldown"]:
            if config["profit_type"] == "money":
                ...
                for _ in range(num_resources):
                    ...
                    inventory[str(user_id)].append(resource)
            ...
            await bot.send_message(
                int(user_id),
                f"🤖 Авто-сбор: {config['emoji']} {config['name']}\n"
                f"Доход собран автоматически!"
            )
    except Exception as e:
        logger.error(f"Ошибка в auto_collect_business для {user_id}: {e}")

async def auto_collect_loop():
    """Цикл авто-сбора бизнесов"""
    while True:
        try:
            users = await load_users()
            for user_id, data in users.items():
                for biz_key, biz_data in data.get("business", {}).items():
                    if biz_data.get("owned", False) and biz_data.get("auto_collect", False):
                        await auto_collect_business(user_id, biz_key)
            await asyncio.sleep(60)  # Проверяем каждую минуту
        except Exception as e:
            logger.error(f"Ошибка в цикле авто-сбора: {e}")
...
async def open_container(callback: types.CallbackQuery):
    ...
    try:
        ...
        # Сохраняем выбранную машину с уникальным ключом
        ...
        # Создаём кнопки для действий с машиной
        keyboard = [
            [
...
async def container_take(callback: types.CallbackQuery):
    ...
    try:
        ...
        # Ищем анимацию с таким item_id
        found = False
        car_data = None
        for uid, data in container_animations.items():
            if data.get("item_id") == item_id and uid == user_id:
                found = True
                car_data = data
                break
        
        if not found:
            await callback.answer("❌ Машина не найдена или уже обработана!", show_alert=True)
            return
        ...
        # Проверяем, есть ли поле "inventory" у пользователя
...
async def business_menu(callback: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        if user_business_count < 1:
            ...
            # Кнопки управления авто-сбором для существующих бизнесов
            for key, config in BUSINESS_CONFIG.items():
...
# ========== ВКЛЮЧЕНИЕ/ВЫКЛЮЧЕНИЕ АВТО-СБОРА ==========
...
async def slots_spin(callback: types.CallbackQuery):
    ...
    try:
        ...
        # Слоты с шансом выигрыша 25%
        symbols = ["🍒", "🍋", "🍊", "🍇", "💎", "7️⃣"]
        result = [random.choice(symbols) for _ in range(3)]
        
        # Проверка выигрыша (25% шанс)
        win = 0
        if random.random() < 0.25:  # 25% шанс на выигрыш
            # Проверяем комбинации
            if result[0] == result[1] == result[2]:
                if result[0] == "7️⃣":
                    win = bet * 5  # Джекпот x5
                elif result[0] == "💎":
                    win = bet * 3  # Алмаз x3
                else:
                    win = bet * 2  # Три одинаковых x2
            elif result[0] == result[1] or result[1] == result[2] or result[0] == result[2]:
                win = bet * 1.5  # Два одинаковых x1.5
            
            # Если выигрыш все еще 0, даем маленький бонус
            if win == 0:
                win = bet * 1.2
...
async def casino_mines(callback: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        mines_count = casino.get("mines_count", 4)  # Изменено с 3 на 4
...
async def mines_settings(callback: types.CallbackQuery, state: FSMContext):
    ...
    try:
        await callback.message.edit_text(
            "✏️ Введите количество мин (от 4 до 10):",  # Изменено с 2 на 4
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def process_mines_count(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        if mines < 4 or mines > 10:  # Изменено с 2 на 4
...
async def mines_cell_click(callback: types.CallbackQuery):
    ...
    try:
        ...
        # Проверяем выигрыш (открыты все безопасные клетки)
...
# ========== ОБРАБОТЧИК ПРОМОКОДОВ (ВСЕГДА В КОНЦЕ) ==========
@dp.message(F.text, ~F.text.startswith('/'))
async def handle_promo(message: types.Message, state: FSMContext):
    # Проверяем, есть ли активное состояние - если есть, то игнорируем
...
async def main():
    ...
    try:
        ...
        # Запускаем цикл авто-сбора
        auto_collect_task = asyncio.create_task(auto_collect_loop())
        logger.info("🤖 Цикл авто-сбора бизнесов запущен!")
