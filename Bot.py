...
# Состояния для покупки/продажи валюты
...
class CurrencyRates:
    def __init__(self):
        self.rates = {
            "BTC": {"price": 150000, "min": 100000, "max": 300000},
            "WETcoin": {"price": 600000, "min": 500000, "max": 5000000},
            "BRcoins": {"price": 50000000, "min": 10000000, "max": 150000000}
        }
    
    def update_rates(self):
        for currency in self.rates:
            change = random.uniform(-0.08, 0.08)
            new_price = self.rates[currency]["price"] * (1 + change)
...
async def casino_menu(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text("🎰 Казино", reply_markup=InlineKeyboardMarkup(inline_keyboard=keyboard))
    await callback.answer()
...
async def roulette_menu(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text("🎡 Рулетка (1000 BRcoins)", reply_markup=InlineKeyboardMarkup(inline_keyboard=keyboard))
    await callback.answer()
...
async def roulette_play(callback: types.CallbackQuery):
    ...
    if user["brcoins"] < 1000:
        await callback.answer("❌ Нужно 1000 BRcoins!", show_alert=True)
        return
    ...
    win = False
    if bet_type in ["red", "black"] and result_color == bet_type:
        win = True
    elif bet_type in ["even", "odd"] and result_parity == bet_type:
        win = True
    
    if win:
        user["brcoins"] += 2000
        result_text = f"🎉 Выигрыш! +1000 BRcoins"
    else:
        result_text = f"😞 Проигрыш! -1000 BRcoins"
    
    save_users(users)
    await callback.message.edit_text(
        f"🎡 {result_text}\nВыпало: {result_number} {result_color}\nБаланс: {user['brcoins']} BRcoins",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 Назад", callback_data="casino")]])
    )
    await callback.answer()
...
async def slots_play(callback: types.CallbackQuery):
    ...
    if user["brcoins"] < 50:
        await callback.answer("❌ Нужно 50 BRcoins!", show_alert=True)
        return
    ...
    if win_multiplier > 0:
        ...
        result_text = f"🎉 Выигрыш {win_amount} BRcoins (x{win_multiplier})!"
    else:
    ...
    await callback.message.edit_text(
        f"🎰 {' | '.join(result)}\n{result_text}\nБаланс: {user['brcoins']} BRcoins",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🎰 Ещё", callback_data="slots")], [InlineKeyboardButton(text="🔙 Назад", callback_data="casino")]])
    )
    await callback.answer()
...
async def coinflip_menu(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text("🪙 Коинфлип (100 BRcoins)", reply_markup=InlineKeyboardMarkup(inline_keyboard=keyboard))
    await callback.answer()
...
async def coinflip_play(callback: types.CallbackQuery):
    ...
    if user["brcoins"] < 100:
        await callback.answer("❌ Нужно 100 BRcoins!", show_alert=True)
        return
    ...
    if win:
        user["brcoins"] += 200
        result_text = "🎉 Выигрыш! +100 BRcoins"
    else:
        result_text = "😞 Проигрыш! -100 BRcoins"
...
async def containers_menu(callback: types.CallbackQuery):
    keyboard = [
        [InlineKeyboardButton(text="🚗 Особый контейнер (БЕСПЛАТНО)", callback_data="container_Особый контейнер")],
        [InlineKeyboardButton(text="🔙 Назад", callback_data="back_main")]
    ]
...
async def open_container(callback: types.CallbackQuery):
    ...
    # Бесплатно - деньги не списываются
...
async def show_inventory(callback: types.CallbackQuery):
    ...
    if not user["inventory"]:
        await callback.message.edit_text("🎒 Инвентарь пуст.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 Назад", callback_data="containers")]]))
        await callback.answer()
        return
    
    text = "🎒 Инвентарь:\n\n" + "\n".join([f"• {item['name']} ({item.get('price', 0):,.0f}₽)" for item in user["inventory"]])
    await callback.message.edit_text(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 Назад", callback_data="containers")]]))
    await callback.answer()
...
async def buy_business_menu(callback: types.CallbackQuery):
    ...
    if not keyboard:
        await callback.message.edit_text("❌ Нет доступных бизнесов!", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 Назад", callback_data="businesses")]]))
        await callback.answer()
        return
    
    await callback.message.edit_text("🏪 Выберите бизнес:", reply_markup=InlineKeyboardMarkup(inline_keyboard=keyboard))
    await callback.answer()
...
async def buy_business(callback: types.CallbackQuery):
    ...
    if business_name == "24/7":
        ...
        if owners >= business.get("max_owners", 25):
            await callback.answer("❌ Все 24/7 куплены!", show_alert=True)
            return
