...
async def work_trucker(callback: types.CallbackQuery):
    if not await check_access(callback):
        return
    user = load_users()[str(callback.from_user.id)]
...
async def work_miner(callback: types.CallbackQuery):
    ...
    if random.random() < 0.1:
        ...
        bonus = f"\n💎 +{br_bonus} BRcoins"
    save_users(load_users())
    await callback.message.edit_text(f"⛏️ +{income:,}₽{bonus}", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 Назад", callback_data="works")]]))
    await callback.answer()
...
# СНАЧАЛА car_sell_ (более конкретный, чтобы не конфликтовал)
...
# ПОТОМ car_ (общий, срабатывает только если не совпало с car_sell_)
...
async def buy_business_menu(callback: types.CallbackQuery):
    ...
    for name, data in BUSINESSES.items():
        if name in business_owners:
            ...
            keyboard.append([InlineKeyboardButton(text=f"❌ {name} - занят ({owner_name})", callback_data=f"biz_info_{name}")])
        else:
            keyboard.append([InlineKeyboardButton(text=f"✅ {name} - {data['price']:,.0f}₽", callback_data=f"biz_choose_{name}")])
    
    keyboard.append([InlineKeyboardButton(text="🔙 Назад", callback_data="businesses")])
    await callback.message.edit_text(
        "🏪 Доступные бизнесы:\n\n⚠️ Каждый бизнес - только 1 игрок!\n✅ доступен, ❌ занят",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=keyboard)
    )
    await callback.answer()
...
async def biz_info(callback: types.CallbackQuery):
    ...
    text = (
        ...
        f"💰 {data['price']:,.0f}₽\n📈 {data['income']:,.0f}₽/сутки\n"
        f"👤 Владелец: {owner_text}\n\n⬆️ Улучшение: +10% дохода за 50к BRcoins/ур."
    )
...
async def buy_business(callback: types.CallbackQuery):
    ...
    business_owners = load_business_owners()
    if business_name in business_owners:
        await callback.answer("❌ Уже куплен!", show_alert=True)
        return
    ...
    business_owners[business_name] = str(callback.from_user.id)
