...
CONTAINERS = {
    ...
    "Итальянский контейнер": {
        "price": 50000000,
        "items": [
            {"name": "BMW M5 F90", "price": 20000000, "chance": 0.55},
            {"name": "Aurus Senat", "price": 45000000, "chance": 0.45},
            {"name": "Ferrari F40", "price": 50000000, "chance": 0.30},
            {"name": "Lamborghini Aventador", "price": 60000000, "chance": 0.25},
            {"name": "Maserati MC20", "price": 40000000, "chance": 0.35}
        ]
    },
    "Спортивный контейнер": {
        "price": 30000000,
        "items": [
            {"name": "Porsche 911", "price": 25000000, "chance": 0.40},
            {"name": "Audi R8", "price": 30000000, "chance": 0.35},
            {"name": "Mercedes AMG GT", "price": 35000000, "chance": 0.30},
            {"name": "Nissan GT-R", "price": 20000000, "chance": 0.45},
            {"name": "Chevrolet Corvette", "price": 28000000, "chance": 0.38}
        ]
    },
    "Люксовый контейнер": {
        "price": 80000000,
        "items": [
            {"name": "Bentley Continental", "price": 70000000, "chance": 0.30},
            {"name": "Rolls-Royce Phantom", "price": 90000000, "chance": 0.25},
            {"name": "Maybach S-Class", "price": 65000000, "chance": 0.35},
            {"name": "Aston Martin DB11", "price": 75000000, "chance": 0.28},
            {"name": "Jaguar F-Type", "price": 55000000, "chance": 0.40}
        ]
    },
    "Премиум контейнер": {
        "price": 150000000,
        "items": [
            {"name": "Koenigsegg Jesko", "price": 300000000, "chance": 0.15},
            {"name": "Pagani Huayra", "price": 250000000, "chance": 0.20},
            {"name": "Lamborghini Veneno", "price": 450000000, "chance": 0.10},
            {"name": "Ferrari LaFerrari", "price": 350000000, "chance": 0.18},
            {"name": "McLaren P1", "price": 200000000, "chance": 0.25}
        ]
    }
}
...
async def works_menu(callback: types.CallbackQuery):
    keyboard = [
        ...
        [InlineKeyboardButton(text="😴 Отдых (снять усталость)", callback_data="rest")],
        [InlineKeyboardButton(text="🔙 Назад", callback_data="back_main")]
    ]
    await callback.message.edit_text(
        "💼 Выберите работу:\n\n"
        "Каждая работа даёт доход, но повышает усталость на 10.\n"
        "При усталости > 70 доход снижается на 50%.",
...
async def work_trucker(callback: types.CallbackQuery):
    ...
    if user["fatigue"] >= 100:
        await callback.answer("❌ Вы слишком устали! Отдохните.", show_alert=True)
        return
    ...
    user["fatigue"] = min(100, user["fatigue"] + 10)
...
async def work_diver(callback: types.CallbackQuery):
    ...
    user["total_earned"] += income
    user["fatigue"] = min(100, user["fatigue"] + 10)
    user["energy"] = max(0, user["energy"] - 8)
...
async def work_farmer(callback: types.CallbackQuery):
    ...
    user["total_earned"] += income
    user["fatigue"] = min(100, user["fatigue"] + 10)
    user["energy"] = max(0, user["energy"] - 10)
...
async def rest(callback: types.CallbackQuery):
    user_id = str(callback.from_user.id)
    users = load_users()
    user = users[user_id]
    
    user["fatigue"] = max(0, user["fatigue"] - 20)
    user["energy"] = min(100, user["energy"] + 20)
    
    save_users(users)
    
    await callback.answer("😴 Вы отдохнули! Усталость снижена на 20", show_alert=True)
    await works_menu(callback)
...
async def cases_menu(callback: types.CallbackQuery):
    keyboard = [
        [InlineKeyboardButton(text="📦 Обычный кейс (100 BRcoins)", callback_data="case_normal")],
        [InlineKeyboardButton(text="✨ Золотой кейс (500 BRcoins)", callback_data="case_gold")],
        [InlineKeyboardButton(text="🔥 Легендарный кейс (2000 BRcoins)", callback_data="case_legendary")],
...
async def open_case(callback: types.CallbackQuery):
    ...
    case_prices = {"normal": 100, "gold": 500, "legendary": 2000}
    ...
    if case_type == "normal":
        items = [
            {"name": "1000 рублей", "type": "money", "value": 1000, "chance": 0.3},
            {"name": "5000 рублей", "type": "money", "value": 5000, "chance": 0.2},
            {"name": "10 BRcoins", "type": "brcoins", "value": 10, "chance": 0.2},
            {"name": "25 BRcoins", "type": "brcoins", "value": 25, "chance": 0.15},
            {"name": "Обычный скин", "type": "skin", "value": "обычный", "chance": 0.15}
        ]
    elif case_type == "gold":
        items = [
            {"name": "100 BRcoins", "type": "brcoins", "value": 100, "chance": 0.3},
            {"name": "250 BRcoins", "type": "brcoins", "value": 250, "chance": 0.2},
            {"name": "Бизнес-лицензия", "type": "license", "value": "business", "chance": 0.25},
            {"name": "Редкая валюта (BTC)", "type": "currency", "value": "BTC", "chance": 0.15},
            {"name": "Редкая валюта (WETcoin)", "type": "currency", "value": "WETcoin", "chance": 0.1}
        ]
    else:
        items = [
            {"name": "Контейнер", "type": "container", "value": "premium", "chance": 0.25},
            {"name": "Ключ от контейнера", "type": "key", "value": "key", "chance": 0.2},
            {"name": "500 BRcoins", "type": "brcoins", "value": 500, "chance": 0.2},
            {"name": "1000 BRcoins", "type": "brcoins", "value": 1000, "chance": 0.15},
            {"name": "Большая сумма (100000₽)", "type": "money", "value": 100000, "chance": 0.2}
        ]
    ...
    result_text = ""
    if selected_item["type"] == "money":
        user["money"] += selected_item["value"]
        result_text = f"💰 Вы получили {selected_item['value']:,.0f} рублей!"
    elif selected_item["type"] == "brcoins":
        user["brcoins"] += selected_item["value"]
        result_text = f"💎 Вы получили {selected_item['value']} BRcoins!"
    else:
        user["inventory"].append(selected_item)
        result_text = f"📦 Вы получили: {selected_item['name']}!"
    
    save_users(users)
    
    await callback.message.edit_text(
        f"📦 Открыт {case_type} кейс!\n\n{result_text}\n"
        f"Ваш баланс: {user['brcoins']} BRcoins",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="📦 Открыть ещё", callback_data=f"case_{case_type}")],
            [InlineKeyboardButton(text="🔙 Назад", callback_data="cases")]
        ])
    )
    await callback.answer()
...
async def show_inventory(callback: types.CallbackQuery):
    ...
    items_text = "\n".join([f"• {item['name']}" for item in user["inventory"]])
...
async def stats_menu(callback: types.CallbackQuery):
    ...
    stats_text = (
        ...
        f"💎 BRcoins: {user['brcoins']}\n"
        f"📦 Открыто кейсов: {user['cases_opened']}\n"
