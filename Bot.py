...
async def work_farmer_menu(callback: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        if last_collect:
            last_time = datetime.fromisoformat(last_collect)
            elapsed = (datetime.now() - last_time).total_seconds()
            if elapsed < 60:  # 1 минута
                ready = False
                remaining = 60 - elapsed
                remaining_text = f"⏳ До следующего сбора: {int(remaining)} сек."
...
async def farm_harvest(callback: types.CallbackQuery):
    ...
    try:
        ...
        if last_collect:
            ...
            if elapsed < 60:
                remaining = 60 - elapsed
                await callback.answer(f"⏳ Подождите {int(remaining)} секунд!", show_alert=True)
                return
...
async def farm_info(callback: types.CallbackQuery):
    ...
    try:
        text = "ℹ️ Ферма\n\n"
        text += "🌾 Вы можете собирать урожай каждые 1 минуту.\n\n"
...
async def process_casino_bet(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await casino_menu(message, state)
    except ValueError:
        await message.answer("❌ Введите число!")
    except Exception as e:
...
async def process_mines_count(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await casino_mines(message, state)
    except ValueError:
        await message.answer("❌ Введите число!")
    except Exception as e:
...
async def mines_play(callback: types.CallbackQuery):
    if not await check_access(callback):
        return
