...
async def buy_business_menu(callback: types.CallbackQuery):
    ...
    for name, data in BUSINESSES.items():
        if name in business_owners:
            ...
            keyboard.append([InlineKeyboardButton(text=button_text, callback_data=f"biz_info_{name}")])
        else:
            keyboard.append([InlineKeyboardButton(text=f"✅ {name} - {data['price']:,.0f}₽", callback_data=f"biz_buy_{name}")])
...
async def biz_buy_menu(callback: types.CallbackQuery):
    business_name = callback.data.split("biz_buy_")[1]
    
    keyboard = [
        [InlineKeyboardButton(text="✅ Купить", callback_data=f"buy_biz_{business_name}")],
...
async def biz_info(callback: types.CallbackQuery):
    business_name = callback.data.split("biz_info_")[1]
    data = BUSINESSES.get(business_name)
