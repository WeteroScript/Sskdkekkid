...
# Курсы валют (обновляются каждые 10 минут)
class CurrencyRates:
    def __init__(self):
        self.rates = {
...
async def buy_business(callback: types.CallbackQuery):
    if not await check_access(callback):
        return
    business_name = callback.data.replace("buy_biz_", "")
    user_id = str(callback.from_user.id)
    users = load_users()
    user = users[user_id]
