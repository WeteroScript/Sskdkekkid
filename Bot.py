...
BUSINESS_CONFIG = {
    "auto_mine": {
        ...
        "resources": ["Красный алмаз", "Цветной алмаз", "Рубин", "Александрит"],
        "cooldown": 3600  # 1 час
    },
    "tech_center": {
...
async def check_business_loop():
    ...
    while business_running:
        try:
            # Проверяем все бизнесы пользователей
            business = await load_business()
            users = await load_users()
                ...
                for biz_key, biz_data in user_business.items():
                    if biz_data.get("owned", False):
                        ...
                        if last_collect:
                            ...
                            # Если бизнес готов, уведомляем пользователя
                            if elapsed >= cooldown:
                                try:
...
async def buy_business(callback: types.CallbackQuery):
    ...
    try:
        ...
        user["total_earned"] -= config["price"]  # Учитываем как траты
        ...
        for biz_key, biz_data in user.get("business", {}).items():
            ...
            if elapsed >= config["cooldown"]:
                ...
                if config["profit_type"] == "money":
                    ...
                    # Для Авто-Шахты - даём ресурсы
                    if user_id not in inventory:
                        inventory[user_id] = []
                    
                    # Даём 1-3 случайных ресурса из списка
