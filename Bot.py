...
class CurrencyRates:
    ...
    def update_rates(self):
        for currency in self.rates:
            rand = random.random()
            if rand < 0.10:  # 10% - падение до 0
                new_price = 0
            elif rand < 0.50:  # 40% - падение до средней
                new_price = self.rates[currency]["avg"] * random.uniform(0.3, 0.9)
            else:  # 50% - взлет до максимальной
...
# Бизнесы (глобальные, только 1 владелец)
BUSINESSES = {
    ...
    "24/7": {"price": 100000000, "income": 50000000, "level_multiplier": 1.1}
}
...
async def containers_menu(callback: types.CallbackQuery):
    keyboard = [
        [InlineKeyboardButton(text="🚗 Особый контейнер (240кк₽)", callback_data="container_Особый контейнер")],
        [InlineKeyboardButton(text="🔙 Назад", callback_data="back_main")]
    ]
...
async def car_menu(callback: types.CallbackQuery):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton(text="💰 Продать государству", callback_data=f"car_sell_{car_index}")],
...
async def buy_business_menu(callback: types.CallbackQuery):
    ...
    for name, data in BUSINESSES.items():
        if name in business_owners:
            ...
            button_text = f"❌ {name} - занят ({owner_name})"
        else:
            button_text = f"✅ {name} - {data['price']:,.0f}₽"
        keyboard.append([InlineKeyboardButton(text=button_text, callback_data=f"buy_biz_{name}")])
