...
# ========== ФЕРМЕРСКИЕ РЕСУРСЫ ==========
FARM_RESOURCES = [
    {"name": "Молоко", "price": 5000, "min": 10, "max": 25},
    {"name": "Сено", "price": 3000, "min": 15, "max": 40},
    {"name": "Яйца", "price": 2000, "min": 20, "max": 50}
]
...
async def get_auto_mine_resource():
    """Получить случайный ресурс из списка авто-шахты с учётом шансов"""
...
async def work_farmer_menu(callback: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        # Проверяем последний сбор урожая
        ...
        if last_collect:
            last_time = datetime.fromisoformat(last_collect)
            elapsed = (datetime.now() - last_time).total_seconds()
            if elapsed < 3600:  # 1 час кулдаун
                ready = False
                remaining = 3600 - elapsed
                hours = int(remaining // 3600)
                minutes = int((remaining % 3600) // 60)
                remaining_text = f"⏳ До следующего сбора: {hours:02d}:{minutes:02d}"
    ...
    try:
        ...
        if last_collect:
            ...
            if elapsed < 3600:
                ...
                await callback.answer(f"⏳ Подождите {minutes} минут!", show_alert=True)
                return
        ...
        # Также добавляем ресурсы в инвентарь для продажи через скупщика
...
async def farm_info(callback: types.CallbackQuery):
    ...
    try:
        text = "ℹ️ Ферма\n\n"
        text += "🌾 Вы можете собирать урожай каждые 1 час.\n\n"
        text += "📦 Ресурсы, которые можно получить:\n"
        text += f"🥛 Молоко - {FARM_RESOURCES[0]['min']}-{FARM_RESOURCES[0]['max']} л.\n"
        text += f"🌿 Сено - {FARM_RESOURCES[1]['min']}-{FARM_RESOURCES[1]['max']} кг.\n"
        text += f"🥚 Яйца - {FARM_RESOURCES[2]['min']}-{FARM_RESOURCES[2]['max']} шт.\n\n"
        text += f"💰 Молоко можно продать за {FARM_RESOURCES[0]['price']:,}₽/л.\n"
        text += f"💰 Сено можно продать за {FARM_RESOURCES[1]['price']:,}₽/кг.\n"
        text += f"💰 Яйца можно продать за {FARM_RESOURCES[2]['price']:,}₽/шт.\n\n"
...
async def process_trade_amount(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        # Проверяем лимит на покупку/продажу
...
async def inventory_main_menu(callback: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        # Собираем все ресурсы (включая фермерские)
        all_resources = MINE_RESOURCES + [
            {"name": "Молоко", "price": 5000},
            {"name": "Сено", "price": 3000},
            {"name": "Яйца", "price": 2000}
        ]
...
async def business_menu(callback: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        # Кнопки покупки для доступных бизнесов
        for key, config in BUSINESS_CONFIG.items():
...
async def buy_business(callback: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        # Проверяем, не владеет ли уже пользователь
        ...
        # Добавляем в глобальный список владельцев
