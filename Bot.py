...
class CurrencyRates:
    ...
    def _do_update(self):
        """Внутренний метод обновления курсов"""
...
async def get_main_menu(user_id):
    ...
    text = (
        ...
        f"⛏️ Осталось попыток: {user['mine_attempts']}/100"
    )
...
async def generate_and_send_promo():
    """Генерирует и отправляет промокод в канал"""
    try:
...
async def promo_auto_loop():
    """Основной цикл автогенерации промокодов"""
...
async def mine_dig(callback: types.CallbackQuery):
    ...
    try:
        ...
        # ДОБАВЛЯЕМ ДЕНЬГИ ЗА ХОДКУ (50к - 150к)
        mine_income = random.randint(50000, 150000)
        user["money"] += mine_income
        user["total_earned"] += mine_income
...
async def process_trade_amount(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        # Отправляем обновленное меню трейдинга через новый callback
        currency_rates.update_rates()
        users = await load_users()
        user = users.get(user_id, get_default_user())
