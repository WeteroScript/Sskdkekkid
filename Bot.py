...
async def buyer_menu(callback: types.CallbackQuery):
    ...
    try:
        ...
        # Кнопка продать все сразу
        keyboard.append([InlineKeyboardButton(
            text="💰 ПРОДАТЬ ВСЕ РЕСУРСЫ",
            callback_data="sell_all_resources_global"
        )])
        keyboard.append([InlineKeyboardButton(text="🔙 Назад", callback_data="back_main")])
        
        await callback.message.edit_text(
            "🏦 Скупщик драгоценностей\n\n"
            "Выберите ресурс для продажи или продайте всё сразу:",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=keyboard)
        )
    except Exception as e:
...
@dp.callback_query(F.data == "sell_all_resources_global")
async def sell_all_resources_global(callback: types.CallbackQuery):
    """Продажа всех ресурсов сразу - БЕЗ ВВОДА КОЛИЧЕСТВА"""
    if not await check_access(callback):
        return
    ...
    try:
        ...
        await callback.message.edit_text(
            f"💰 ПРОДАНО ВСЕ РЕСУРСЫ!\n"
            f"📦 Продано: {total_items} предметов\n"
...
async def sell_all_resources(callback: types.CallbackQuery):
    """Продажа всех ресурсов одного типа - БЕЗ ВВОДА КОЛИЧЕСТВА"""
    if not await check_access(callback):
        return
...
async def buy_business_exec(callback: types.CallbackQuery):
    ...
    try:
        ...
        business_owners[business_name].append(user_id)
        ...
        await callback.message.edit_text(
            f"✅ Бизнес куплен!\n\n"
            f"🏪 {business_name}\n"
            f"💰 Доход: {business['income']:,.0f}₽/сутки\n"
            f"⬆️ Уровень: 1\n\n"
            f"Не забывайте собирать доход каждый день!",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="🔙 В меню бизнесов", callback_data="businesses")]
            ])
        )
    except Exception as e:
...
async def biz_info(callback: types.CallbackQuery):
    ...
    try:
        ...
        text = (
            ...
            f"👤 Владельцев: {owner_count}/{max_owners}\n"
            f"⬆️ Улучшение: +10% дохода\n"
            f"💎 Стоимость улучшения: 50,000 BRcoins"
        )
...
async def collect_income(callback: types.CallbackQuery):
    ...
    try:
        ...
        if hours_passed < 24:
            await callback.answer(
                f"⏳ Доход ещё не накопился!\n"
                f"Осталось: {24 - int(hours_passed)} ч",
                show_alert=True
            )
            return
...
async def upgrade_business(callback: types.CallbackQuery):
    ...
    try:
        ...
        await callback.message.edit_text(
            ...
            f"💰 Новый доход: {new_income:,.0f}₽/сутки\n"
